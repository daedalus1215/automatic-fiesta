...
export const fetchTag = async (tagId: string): Promise<Tag[]> => {
  const response = await axios.get(`${config.api}tags/${tagId}`);
  return response.data;
};

export const putTag = async (tagId: string): Promise<Tag[]> => {
  // const response = await axios.put(`${config.api}tags/${tagId}`)
  // return response.data
  throw new Error('Implement putTag');
};
